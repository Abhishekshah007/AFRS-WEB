        ...
        <AnimateOnScroll>
          <div className="grid gap-6 lg:grid-cols-[1fr_1.2fr] lg:items-end mb-10 md:mb-12">
            ...
            <p className={`text-sm sm:text-base ${aboutTokens.body}`}>
              AFRS maintains rigorous standards through accredited programmes, laboratory protocols,
              and professional memberships recognized across the forensic science community.
            </p>
          </div>
        </AnimateOnScroll>
            ...
            {items.map((item) => (
              <li key={item.title}>
                ...
                >
                  ...
                  <h3 className="mt-4 font-extrabold text-[var(--about-text)] text-sm leading-snug">
                    {item.title}
                  </h3>
                  <p className={`mt-4 text-sm ${aboutTokens.body}`}>{item.description}</p>
                </article>
              </li>
            ))}
          </ul>
