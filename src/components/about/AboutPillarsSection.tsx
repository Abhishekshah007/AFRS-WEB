...
function PillarPanel({ title, eyebrow, items, icon, gradient }: Readonly<PillarProps>) {
  return (
    <article className="relative flex h-full flex-col overflow-hidden rounded-3xl border border-slate-100 bg-white p-6 sm:p-8 shadow-[0_10px_30px_rgba(15,23,42,0.06)] card-pop">
      ...
      <ul className="mt-8 flex flex-1 flex-col gap-3">
        {items.map((item) => (
          <li
            key={item}
            className="flex flex-1 items-start gap-3 rounded-xl border border-slate-100 bg-slate-50/80 px-4 py-3 text-sm leading-relaxed text-slate-700 text-justify"
          >
