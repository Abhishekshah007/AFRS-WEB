...
const researchItems = [
  { num: '01', title: 'Forensic Toxicology Research', desc: 'Novel detection methods for controlled substances in biological matrices.' },
  { num: '02', title: 'Digital Evidence Standards', desc: 'Developing admissibility frameworks for mobile and cloud forensics.' },
  { num: '03', title: 'Crime Scene Reconstruction', desc: '3D modelling and trajectory analysis for complex investigations.' },
]
...
export function ServicesPageView({ catalogItems, directors, teamMembers, site, totalVisitors }: Props) {
  ...
  return (
    <div className="afsl-page bg-white">
      ...
      <section className="afsl-hero relative overflow-hidden text-white">
        ...
        <div className={`${CONTAINER} relative z-10 py-16 lg:py-24 grid gap-12 lg:grid-cols-[1fr_400px] items-center`}>
          <div>
            ...
            <h1 className="mt-4 text-4xl sm:text-5xl lg:text-[56px] font-extrabold leading-[1.08] tracking-tight reveal-up-d1">
              Future-Ready{' '}
              <span className="text-[#fdb022]">Forensic</span>
            </h1>
            <p className="mt-5 max-w-lg text-base text-white/80 leading-relaxed reveal-up-d2">
              AFSL delivers high-end forensic laboratory solutions — from crime scene investigation to digital
              evidence analysis — with ISO-compliant methodology and court-ready reporting.
            </p>
          ...
          <div className="reveal-up-d2">
            <HeroInquiryCard services={catalogItems.map((c) => ({ title: c.title, slug: c.slug }))} />
          </div>
        </div>
      </section>
        ...
        <div className={CONTAINER}>
          <AnimateOnScroll>
            <div className="grid gap-10 lg:grid-cols-2 items-center">
              <div>
                <h2 className="text-2xl sm:text-3xl font-extrabold text-[#0f172a]">About AFSL Laboratory</h2>
                <p className="mt-5 text-slate-600 text-sm leading-relaxed">
                  The Applied Forensic Science Laboratory (AFSL) bridges academic forensic research with
                  industry-standard investigation. Our facilities feature cutting-edge instruments for fingerprint
                  analysis, digital evidence recovery, chemical toxicology, and biological profiling.
                </p>
                <p className="mt-4 text-slate-600 text-sm leading-relaxed">
                  With a commitment to evidence integrity and methodological rigor, AFSL serves as a practical
                  training hub for students and professionals under certified forensic scientists.
                </p>
          ...
          <AnimateOnScroll stagger>
            <div className="grid gap-6 md:grid-cols-2">
              <div className="rounded-[24px] bg-[#0f172a] p-8 sm:p-10 text-white card-pop min-h-[220px] flex flex-col">
                <div className="h-11 w-11 rounded-xl bg-white/10 flex items-center justify-center text-lg mb-5">🔭</div>
                <p className="text-[11px] font-bold uppercase tracking-widest text-violet-300">Laboratory Vision</p>
                <p className="mt-4 text-sm text-white/75 leading-relaxed flex-1">
                  To be India&apos;s most trusted forensic laboratory — setting benchmarks in evidence integrity,
                  scientific accuracy, and professional training for the next generation of investigators.
                </p>
                <Link href="/about" className="mt-6 text-sm font-bold text-[#fdb022] hover:underline">
                  Read More →
                </Link>
              </div>
              <div className="rounded-[24px] border border-slate-100 bg-white p-8 sm:p-10 shadow-sm card-pop min-h-[220px] flex flex-col">
                <div className="h-11 w-11 rounded-xl bg-violet-100 text-[#7c3aed] flex items-center justify-center text-lg mb-5">🎯</div>
                <p className="text-[11px] font-bold uppercase tracking-widest text-[#7c3aed]">Our Mission</p>
                <p className="mt-4 text-sm text-slate-600 leading-relaxed flex-1">
                  Deliver court-admissible forensic analysis, empower law enforcement with actionable insights, and
                  advance forensic education through research-led training and internship programmes.
                </p>
                <Link href="/about" className="mt-6 text-sm font-bold text-[#7c3aed] hover:underline">
                  Read More →
                </Link>
              </div>
            </div>
              ...
              {directors.map((d) => (
                ...
                >
                  <div className="mx-auto h-24 w-24 rounded-full overflow-hidden bg-gradient-to-br from-[#7c3aed] to-[#4c1d95] text-white flex items-center justify-center text-2xl font-bold shadow-lg ring-4 ring-violet-100">
                    {d.photo ? (
                      <Image src={d.photo} alt={d.name} width={96} height={96} className="h-full w-full object-cover" />
                    ) : (
                      d.initials
                    )}
                  </div>
                  ...
                  <div className="mt-3 flex justify-center gap-0.5 text-[#fdb022]" aria-label="5 star rating">
                    {'★★★★★'.split('').map((s, i) => (
                      <span key={i} className="text-sm">{s}</span>
                    ))}
                  </div>
                  {d.bio && <p className="mt-3 text-xs text-slate-500 leading-relaxed line-clamp-2">{d.bio}</p>}
                </article>
              ))}
            </div>
          </AnimateOnScroll>
              ...
              {teamMembers.map((m) => (
                <div key={m.name} className="text-center w-[88px] sm:w-[100px]">
                  <div className="mx-auto h-16 w-16 sm:h-[72px] sm:w-[72px] rounded-full overflow-hidden bg-slate-200 ring-2 ring-white shadow-md flex items-center justify-center text-sm font-bold text-[#7c3aed]">
                    {m.photo ? (
                      <Image src={m.photo} alt={m.name} width={72} height={72} className="h-full w-full object-cover" />
                    ) : (
                      m.initials
                    )}
                  </div>
            ...
            <div className="flex flex-wrap justify-center gap-6 sm:gap-10">
              {certifications.map((c) => (
                <div key={c.label} className="text-center">
                  ...
                  <p className="mt-2 text-[10px] font-semibold text-slate-500 max-w-[80px]">{c.label}</p>
                </div>
              ))}
            </div>
          </AnimateOnScroll>
        ...
        <div className={CONTAINER}>
          <AnimateOnScroll>
            <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 mb-12">
              <div>
                <h2 className="text-2xl sm:text-[30px] font-extrabold text-[#0f172a]">Forensic Service Catalog</h2>
                <p className="mt-2 text-slate-500 text-sm max-w-xl">
                  Professional forensic capabilities for investigation, education, and evidence reporting.
                </p>
              </div>
              <Link href="/contact" className="text-sm font-bold text-[#7c3aed] hover:underline shrink-0">
                View All Services →
              </Link>
            </div>
          </AnimateOnScroll>
              ...
              {catalogItems.map((item) => (
                ...
                >
                  ...
                  <div className="flex flex-1 flex-col p-6">
                    ...
                    {item.desc && (
                      <p className="mt-3 text-sm text-slate-500 leading-relaxed line-clamp-3 flex-1">{item.desc}</p>
                    )}
                    <span className="mt-5 inline-flex items-center gap-1 text-sm font-bold text-[#7c3aed]">
                      READ MORE <span aria-hidden className="transition-transform group-hover:translate-x-1">→</span>
                    </span>
                  </div>
                </Link>
              ))}
            ...
            <div className="rounded-[24px] bg-[#0f172a] text-white p-8 sm:p-12 grid gap-8 lg:grid-cols-[1fr_1.2fr] items-center">
              <div>
                ...
                <p className="mt-4 text-sm text-white/70 leading-relaxed max-w-md">
                  Court-ready expert opinions and investigative support for law firms, agencies, and corporate clients.
                </p>
                <Link
                  href="/contact"
              ...
              <ul className="grid sm:grid-cols-2 gap-x-6 gap-y-3">
                {legalServices.map((svc) => (
                  <li key={svc} className="flex items-start gap-2.5 text-sm text-white/90">
                    <span className="mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-[#7c3aed] text-[10px]">✓</span>
                    {svc}
                  </li>
                ))}
              </ul>
        ...
        <div className={CONTAINER}>
          <AnimateOnScroll>
            <div className="text-center max-w-2xl mx-auto mb-12">
              <h2 className="text-2xl sm:text-[30px] font-extrabold text-[#0f172a]">AFSL Professional Kits</h2>
              <p className="mt-3 text-slate-500 text-sm">
                Field-ready forensic kits designed for evidence integrity and chain-of-custody compliance.
              </p>
            </div>
          </AnimateOnScroll>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-5 mb-8">
              {professionalKits.map((kit) => (
                ...
                >
                  ...
                  <Link href="/contact" className="mt-3 inline-block text-xs font-bold text-[#7c3aed] hover:underline">
                    BUY NOW
                  </Link>
                </div>
              ))}
        ...
        <div className={CONTAINER}>
          <div className="grid gap-8 lg:grid-cols-2">
            <AnimateOnScroll direction="left">
              <div>
                <p className="text-[11px] font-bold uppercase tracking-widest text-violet-300">Training & Internship</p>
                ...
                <div className="mt-8 space-y-5">
                  {[
                    ...
                    >
                      ...
                      <Link href={block.href} className="mt-4 inline-block text-sm font-bold text-[#fdb022] hover:underline">
                        Apply Now →
                      </Link>
                    </div>
                  ))}
            ...
            <AnimateOnScroll direction="right">
              <div>
                <p className="text-[11px] font-bold uppercase tracking-widest text-violet-300">Resources & Projects</p>
                <h2 className="mt-3 text-2xl font-extrabold">Research and Projects</h2>
                <ol className="mt-8 space-y-6">
                  {researchItems.map((item) => (
                    <li key={item.num} className="flex gap-5">
                      <span className="text-3xl font-extrabold text-[#7c3aed]/80 tabular-nums">{item.num}</span>
                      <div>
            ...
            <div className="rounded-[28px] bg-white shadow-xl border border-slate-100 overflow-hidden grid lg:grid-cols-[1fr_1.1fr]">
              <div className="p-8 sm:p-10 lg:p-12 bg-gradient-to-br from-violet-50/80 to-white">
                ...
                <p className="mt-4 text-sm text-slate-600 leading-relaxed">
                  Submit your case details and our forensic team will review your inquiry within 24 business hours.
                </p>
                <div className="mt-10 space-y-5">
                  ...
                  >
                    ...
                    <div>
                      ...
                      <p className="text-sm font-bold text-slate-800 group-hover:text-[#7c3aed] transition">{site.phone}</p>
                    </div>
                  </a>
                  <a href={`mailto:${site.email}`} className="flex items-center gap-4 group">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-bold text-slate-800 group-hover:text-[#7c3aed] transition">{site.email}</p>
                    </div>
                  </a>
                </div>
              </div>
              ...
              <div className="p-8 sm:p-10 lg:p-12 border-t lg:border-t-0 lg:border-l border-slate-100">
                <LabInquiryForm serviceOptions={serviceOptions.length > 0 ? serviceOptions : ['General forensic inquiry']} />
              </div>
            </div>
          </AnimateOnScroll>
        </div>
          ...
          <AnimateOnScroll>
            <div className="grid gap-6 lg:grid-cols-[1.4fr_1fr] items-stretch">
              ...
              <div
                ...
                style={{ background: 'linear-gradient(145deg, #6d28d9 0%, #4c1d95 50%, #7c3aed 100%)' }}
              >
                <p className="text-xs font-bold uppercase tracking-widest text-white/70">Total Cases Solved</p>
