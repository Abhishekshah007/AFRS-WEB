...
export function ReadNextSection({ nextArticle }: ReadNextSectionProps) {
  ...
  return (
    <section
      className="bg-slate-50/90 border-t border-slate-100 py-12 md:py-14"
      aria-labelledby="read-next-heading"
    >
      <div className={`${studentHubTokens.container} max-w-3xl text-center`}>
        <p className="text-[11px] font-bold uppercase tracking-widest text-[var(--articles-primary)]">
          Up Next
        </p>
        <h2
          id="read-next-heading"
          className="mt-3 text-xl sm:text-2xl font-extrabold text-slate-900 leading-snug"
        >
          {nextArticle.title}
        </h2>
